struct OrderService {
    ...
    func place(address: String) async throws -> OrderResponse {
        ...
        let wrapped: Wrapped = try await api.send(OrderEndpoint.place(address: address))
    ...
    func fetchAll() async throws -> [OrderResponse] {
        ...
        let wrapped: Wrapped = try await api.send(OrderEndpoint.list)
    ...
    func fetch(id: String) async throws -> OrderResponse {
        typealias Wrapped = ApiResponse<OrderResponse>
        let w: Wrapped = try await api.send(OrderEndpoint.detail(id: id))
